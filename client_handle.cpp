...
void request_handle(struct Gthread_pool * pool)
{
	...
	int reuse = 1;
	assert(setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(int)) == 0);  // make addr reuse available. by Kingsley 2019.1.26
	...
	char * ptr;
	if( (ptr = getenv("LISTENQ")) != NULL)  //getenv：搜索参数所指向的环境字符串
	{
#if DEBUG == 1
	...
	int r = socketpair(PF_UNIX, SOCK_STREAM, 0, pipefd);//socketpair：创建一对互相连接的unnamed socket
	assert(r != -1);
	setNoBlock(pipefd[1]);
	...
	while(run == true)
	{
		...
		for(int i = 0; i < ret; i++)
		{
			if(events[i].data.fd == listen_fd)//a new client 
			{
				...
				setNoBlock(client_fd);  // by Kingsley 1019.1.26
...
{ 
	...
	char ifClose;                  //if client close itself, service will get 0 return. by Kingsley 2019.1.26
	int ret = recv(client_fd, &ifClose, 1, MSG_PEEK);
	
	if(ret == 0)
	{
		close_client(client_fd);
#if KINGDEBUG == 1
		printf("client closed itself\n");
		return NULL;
#endif
	}
...
#if DEBUG == 1
	printf("%s", rcv_buffer); //GET /index.html HTTP/1.0
#endif
	int i = 0, j = 0;
...
void cannot_execute(int client)
{
 ...
 send(client, buf, strlen(buf), MSG_NOSIGNAL);     //revise send flag from 0 to MSG_NOSIGNAL. by Kingsley 2019.1.26
 sprintf(buf, "Content-type: text/html\r\n");
 send(client, buf, strlen(buf), MSG_NOSIGNAL);
 sprintf(buf, "\r\n");
 send(client, buf, strlen(buf), MSG_NOSIGNAL);
