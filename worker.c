...
int Gthread_pool_init(struct Gthread_pool * pool, int max_tasks, int max_workers, int min_workers)
{
	...
	pthread_mutex_init(&(pool->info_lock), NULL); //互斥锁的初始化
#if DEBUG == 1
	pthread_mutex_init(&(pool->IO_lock), NULL);
#endif

	sem_init(&(pool->surplus_task_num), 0, 0);  //初始化一个信号量
...
void * worker_routline(void * arg)
{
	...
	while(1)
	{
		...
		this_worker->state = READY;  //some thread is BUSY before it goes on. Still don't figure it out. By Kingsley 2019.1.26
		...
		pthread_mutex_lock(&(this_worker->worker_lock));   
		if(!(this_worker->worker_active))	   //thread exit flag. In the former edition, pthread_exit will cause pthread_cond_wait error. By Kingsley 2019.1.26
			{
				pthread_mutex_unlock(&(this_worker->worker_lock));
				pthread_exit(NULL);
			}
...
{
	pthread_mutex_lock(&(worker_to_del->worker_lock));
	worker_to_del->worker_active = false;
	pthread_cond_signal(&(worker_to_del->worker_cond));  //wake up the worker to exit. By Kingsley 2019.1.26
	pthread_mutex_unlock(&(worker_to_del->worker_lock));
	
	//pthread_kill(worker_to_del->id, SIGUSR1);
#if KINGDEBUG
	printf("waiting for exit, %p\n", worker_to_del->id);
#endif
